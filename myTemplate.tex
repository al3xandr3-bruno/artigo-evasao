...
\title{Aprendizado de Máquina Aplicado à Predição da Evasão no Ensino Médio em São Paulo}

% If the manuscript is written in English, then this element must be removed.
\titleinenglish{Machine Learning Applied to Predicting Dropout Rates in High Schools in São Paulo}

% If the manuscript is written in English, then this element must be removed.
\titleinspanish{Aprendizaje automático aplicado a la predicción del abandono escolar en la enseñanza secundaria de São Paulo}
...
Conforme discorre Franceschi (2019), embasado por Castro e Braga (2011), a matriz principal da matriz representa os valores corretos preditos pelo modelo.
